...
func (c *Cluster) reuse(cctx *testcontext.Context) error {
	...
	for _, node := range clients {
		cctx.Log.Debugw("discovered existing bootnode", "id", node.Name)
	}
	...
	for _, node := range clients {
		cctx.Log.Debugw("discovered existing smesher", "id", node.Name)
	}
	c.clients = append(c.clients, clients...)
	...
	for _, poet := range c.poets {
		cctx.Log.Debugw("discovered existing poets", "id", poet.Name)
	}
